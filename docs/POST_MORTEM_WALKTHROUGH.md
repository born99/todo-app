...
- **Native OS Auto-Start**: Integrated `tauri-plugin-autostart` to automatically inject the application into the Windows Registry / Mac LaunchAgent on the very first boot. The application now silently starts in the background as a persistent daemon whenever the computer is turned on, exactly like an antivirus.
